...
.cron.supportedModes[`ticking]: `.cron.enableTicking;
.cron.supportedModes[`tickless]:`.cron.enableTickless;
...
.cron.init:{
    ...
    if[not .cron.cfg.mode in key .cron.supportedModes;
...
.cron.i.setNextTick:{
    ...
    /  * Has a 1ms buffer after 'nextRunTime' for rounding
    timer:.cron.maxTimerAsTimespan & .cron.oneMsAsTimespan | (nextRun + .cron.oneMsAsTimespan) - .time.roundTimestampToMs .time.now[];
